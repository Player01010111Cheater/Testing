...
local function fetchGitHubFile()
    ...
    if success and response and response.StatusCode == 200 then
        ...
        if data.content then
            keys_data = HttpService:JSONDecode(crypt.base64decode(data.content))
            current_sha = data.sha
            print("Файл keys.json загружен, SHA:", current_sha)
            print("Данные keys_data:", HttpService:JSONEncode(keys_data))
        else
            warn("Ошибка: Нет содержимого в ответе GitHub")
            keys_data = {}
        end
    else
        warn("Ошибка загрузки keys.json: Код", response and response.StatusCode or "N/A", "Сообщение:", response and response.StatusMessage or tostring(response))
...
local function updateGitHubFile(new_data)
    ...
    if success and response and response.StatusCode == 200 then
        ...
        if response_data and response_data.content and response_data.content.sha then
            ...
            warn("Ошибка: Неверная структура ответа от GitHub")
